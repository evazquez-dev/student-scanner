...
function parseStaffRows(raw){return String(raw||'').split(/\r?\n/).map((line)=>line.trim()).filter(Boolean).map((line)=>{const parts=line.split('|').map((v)=>v.trim());return{staff_name:parts[0]||'',staff_email:(parts[1]||'').toLowerCase(),location:parts[2]||''};}).filter((row)=>row.staff_email&&row.staff_email.includes('@'));}
function serializeStaffRows(rows){return (Array.isArray(rows)?rows:[]).map((s)=>`${s.staff_name||''} | ${s.staff_email||''} | ${s.location||''}`).join('\n');}
