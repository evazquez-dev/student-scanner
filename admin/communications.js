...
function renderCampaigns(data){
  ...
  campaignsEl.innerHTML=campaigns.map(c=>{
    ...
    const ownerPanel=owners.length ? `<div class="miniPanel"><strong>Completion by advisor / owner</strong>${owners.map(r=>`<div class="miniRow"><div><strong>${esc(r.owner_name||r.owner_email||'Unassigned')}</strong><div class="muted small">${esc(r.owner_email||'')}</div></div><div class="mono">${Number(r.complete||0)}/${Number(r.expected||0)} · ${Number(r.missing||0)} missing</div></div>`).join('')}</div>` : '<div class="miniPanel empty">No owner breakdown is available for this scope.</div>';
    const missingPanel=missing.length ? `<div class="miniPanel"><strong>Missing students</strong>${missing.map(r=>`<div class="miniRow"><div>${studentLink(r)}<div class="muted small">Grade ${esc(r.grade||'—')}${r.owner_name?` · ${esc(r.owner_name)}`:''}</div></div><div class="rowActions"><a class="btn small primary" href="${esc(studentUrl(r,{action:'log-communication',category:campaign.category}))}">Log</a></div></div>`).join('')}${Number(counts.missing||0)>missing.length?`<div class="muted small">Showing first ${missing.length} of ${Number(counts.missing||0)} missing students.</div>`:''}</div>` : '<div class="miniPanel empty">Everyone in this scope is complete.</div>';
    return `<article class="campaign">
      <div class="campaignHead"><div><div class="campaignTitle">${esc(campaign.name||campaign.category||'Required Communication')}</div><div class="muted small">${esc(campaign.category||'')} · ${esc(campaign.start_date||'')} → ${esc(campaign.due_date||'')}</div></div>${campaignStatusTag(c)}</div>
