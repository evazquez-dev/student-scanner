...
const ESAS_GUEST_SESSION_KEY = 'esas_guest_session_v1';
const ESAS_GUEST_SESSION_PREFIX = 'esasg_';
const SESSION_KEYS = [
...
function getStoredEsasGuestSid(){
  try{
    const sid = String(sessionStorage.getItem(ESAS_GUEST_SESSION_KEY) || '').trim();
    return sid.startsWith(ESAS_GUEST_SESSION_PREFIX) ? sid : '';
  }catch{}
  return '';
}

function setStoredEsasGuestSid(sid){
  const value = String(sid || '').trim();
  if (!value.startsWith(ESAS_GUEST_SESSION_PREFIX)) return;
  try{ sessionStorage.setItem(ESAS_GUEST_SESSION_KEY, value); }catch{}
}

function clearStoredEsasGuestSid(){
  try{ sessionStorage.removeItem(ESAS_GUEST_SESSION_KEY); }catch{}
}

function consumeEsasGuestSidFromHash(){
  try{
    const raw = String(location.hash || '').replace(/^#/, '');
    if (!raw) return '';
    const params = new URLSearchParams(raw);
    const sid = String(params.get('esas_access') || '').trim();
    if (!sid.startsWith(ESAS_GUEST_SESSION_PREFIX)) return '';
    setStoredEsasGuestSid(sid);
    params.delete('esas_access');
...
function isEsasApiPath(path){
  try{
    const u = new URL(path, API_BASE);
    return u.pathname.startsWith('/admin/esas/');
  }catch{}
  return String(path || '').startsWith('/admin/esas/');
}

function isEsasGuestMode(){
  return !!getStoredEsasGuestSid() || SESSION?.guest === true;
}
...
async function adminFetch(path, init = {}){
  ...
  const guestSid = isEsasApiPath(path) ? getStoredEsasGuestSid() : '';
  const sid = guestSid || getStoredAdminSessionSid();
...
async function refreshAll({ silent = false } = {}){
  ...
  try{
    ...
    if (isEsasGuestMode() && [401,403,410].includes(Number(error?.status || 0))) {
      stopPolling();
      clearStoredEsasGuestSid();
      appShell.hidden = true;
      loginCard.hidden = false;
      loginOut.textContent = Number(error?.status || 0) === 410
        ? 'This temporary ESAS link is no longer active because the incident ended. You can still sign in normally.'
        : 'This temporary ESAS link expired or is no longer valid. You can still sign in normally.';
...
async function boot(){
  consumeEsasGuestSidFromHash();
  let guestFailureMessage = '';
  ...
  const guestSid = getStoredEsasGuestSid();
  if (guestSid) {
    try{
      const status = await getJson('/admin/esas/status', { method:'GET' });
      SESSION = {
      ...
      clearStoredEsasGuestSid();
      guestFailureMessage = Number(error?.status || 0) === 410
        ? 'This temporary ESAS link is no longer active because the incident ended. Sign in normally if you still need EagleNEST.'
        : 'This temporary ESAS link expired or is invalid. Sign in normally if you still need EagleNEST.';
    }
  }
  ...
  try{
    ...
    appShell.hidden = true;
    loginCard.hidden = false;
    loginOut.textContent = guestFailureMessage || 'Sign in with your school account.';
    await initGoogleLogin();
  }
}
