...
function setBusy(v){BUSY=!!v;for(const b of [enableBtn,disableBtn,refreshBtn,testBtn])b.disabled=BUSY;for(const el of document.querySelectorAll('.prefToggle')){const eligible=el.dataset.eligible==='1';const available=el.dataset.available==='1';el.disabled=BUSY||!eligible||!available;}}
...
async function boot(){enableBtn.addEventListener('click',enableNotifications);disableBtn.addEventListener('click',disableNotifications);refreshBtn.addEventListener('click',()=>{setBusy(true);refreshState().catch(e=>showNotice(e?.message||e,'bad')).finally(()=>setBusy(false));});testBtn.addEventListener('click',sendTest);prefList?.addEventListener('change',(ev)=>{const el=ev.target?.closest?.('.prefToggle');if(!el)return;savePreference(String(el.dataset.prefKey||''),!!el.checked);});ACCESS=await getAccess();if(!ACCESS){loginOut.textContent='Please sign in.';const gsi=await waitForGoogle();gsi.initialize({client_id:GOOGLE_CLIENT_ID,ux_mode:'popup',callback:async(resp)=>{try{loginOut.textContent='Signing in…';await doLogin(resp.credential);location.reload();}catch(e){loginOut.textContent=`Login failed: ${e.message||e}`;}}});gsi.renderButton($('g_id_signin'),{theme:'outline',size:'large'});return;}if(!ACCESS?.can?.notifications&&!ACCESS?.can?.teacher_attendance&&!ACCESS?.can?.admin)throw new Error('forbidden');loginCard.hidden=true;app.hidden=false;viewerPill.textContent=ACCESS.email||'Staff';await refreshState();}
