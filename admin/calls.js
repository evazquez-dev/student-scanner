...
async function boot(){ACCESS=await getAccess();if(!ACCESS){$('loginOut').textContent='Please sign in.';const g=await waitGoogle();g.initialize({client_id:GOOGLE_CLIENT_ID,ux_mode:'popup',callback:async r=>{try{await login(r.credential);location.reload()}catch(e){$('loginOut').textContent=e.message||e}}});g.renderButton($('g_id_signin'),{theme:'outline',size:'large'});return}if(!ACCESS.can?.phone_dashboard)throw new Error('phone_dashboard_extension_or_office_access_required');if(ACCESS.role==='super_admin'&&document.getElementById('fleetNav'))document.getElementById('fleetNav').hidden=false;$('loginCard').hidden=true;$('app').hidden=false;restoreRecentBridge();renderRecent();$('daysSelect').onchange=()=>loadHistory();$('searchInput').oninput=renderRecent;$('closeNote').onclick=closeNotes;$('noteBackdrop').onclick=e=>{if(e.target===$('noteBackdrop'))closeNotes()};$('noteText').oninput=()=>{if(MODAL)putDraft(MODAL,MODAL_KIND,$('noteText').value)};$('clearNote').onclick=()=>{if(MODAL){$('noteText').value='';putDraft(MODAL,MODAL_KIND,'')}};$('saveNote').onclick=saveCommunication;await Promise.all([loadConfig(),loadHistory()]);await loadPhonePilot();connectLive();setInterval(tick,1000);setInterval(()=>loadHistory(false),30000)}
