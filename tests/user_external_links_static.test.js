...
test('Cloudflare Worker uses modular entry and extracted external-links modules', () => {
  ...
  const adminSessionRoute = read('cf-redcake/red-cake-77d5/src/routes/admin-session.js');
  ...
  assert.match(entry, /'\/admin\/access'/);
  assert.match(entry, /path === '\/admin\/user_external_links'/);
  assert.match(entry, /return baseWorker\.fetch\(req, env, ctx\)/);

  // /admin/access moved into the extracted admin-session route. External Links
  // now authenticates through the shared modular bridge instead of asking the
  // legacy Worker for /admin/access.
  assert.match(adminSessionRoute, /path === '\/admin\/access'/);
  assert.match(adminSessionRoute, /buildAdminAccessData/);
