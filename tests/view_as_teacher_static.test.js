...
test('View as Teacher keeps actor identity and evaluates selected staff as effective session identity', () => {
  assert.match(worker, /actor_email:\s*actorEmail/);
  assert.match(worker, /view_as_email:\s*viewAsMode \? effectiveEmail/);
  assert.match(worker, /effectiveRole = viewAsMode \? await emailRole\(env, effectiveEmail\)/);
  assert.match(worker, /path === "\/admin\/session\/view_as"/);
  assert.match(worker, /path === "\/admin\/view_as\/staff"/);
});
...
test('View as Teacher is centrally read-only with only session controls exempted', () => {
  ...
  assert.match(worker, /view_as_teacher_start/);
  assert.match(worker, /view_as_teacher_end/);
});
